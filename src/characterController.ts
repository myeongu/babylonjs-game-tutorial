...
export class Player extends TransformNode {
    ...
    private _updateFromControls(): void {
        ...
        let move1 = correctedHorizontal.addInPlace(correctedVertical);
        let move2 = correctedVertical.addInPlace(correctedHorizontal);

        // clear y so that the character doesn't fly up, normalize for next step
        this._moveDirection = new Vector3((move1).normalize().x, 0, (move2).normalize().z);

        // clamp the input value so that diagonal movement ins't twice as fast
